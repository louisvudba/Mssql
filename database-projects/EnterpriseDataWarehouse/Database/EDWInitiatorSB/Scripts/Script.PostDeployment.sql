...
DECLARE @fileInput VARCHAR(150) = '$(TargetCertPath)'
EXEC master.dbo.xp_fileexist @fileInput, @isExists OUTPUT
IF (@isExists = 1) 
    CREATE CERTIFICATE [TargetCertificate] 
       AUTHORIZATION [TargetUser]
       FROM FILE = '$(TargetCertPath)'
ELSE
    PRINT '$(TargetCertPath) is not existed. Please re-do manually'
GO

BACKUP CERTIFICATE [InitiatorCertificate]
  TO FILE = '$(InitiatorCertPath)'
GO

CREATE ROUTE [//WalletAccount/Route/Target]
    WITH 
        SERVICE_NAME = N'//WalletAccount/Service/Target',
        ADDRESS = '$(TargetTCP)';
CREATE ROUTE [//WalletTransaction/Route/Target]
    WITH 
        SERVICE_NAME = N'//WalletTransaction/Service/Target',
        ADDRESS = '$(TargetTCP)';
CREATE ROUTE [//WalletProperty/Route/Target]
    WITH 
        SERVICE_NAME = N'//WalletProperty/Service/Target',
        ADDRESS = '$(TargetTCP)';
...
CREATE ROUTE [//WalletAccount/Route/Source]
    WITH 
        SERVICE_NAME = '//WalletAccount/Service/Source',
        ADDRESS = N'LOCAL';
CREATE ROUTE [//WalletTransaction/Route/Source]
    WITH 
        SERVICE_NAME = '//WalletTransaction/Service/Source',
        ADDRESS = N'LOCAL';
CREATE ROUTE [//WalletProperty/Route/Source]
    WITH 
        SERVICE_NAME = '//WalletProperty/Service/Source',
        ADDRESS = N'LOCAL';